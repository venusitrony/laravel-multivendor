                    ...
                    <div class="card" style="padding: 30px;margin-top:50px">
                        ...
                        <form action="{{ route('role.update',$role->id) }}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="name" value="{{ $role->name }}" class="form-control" id="name" aria-describedby="emailHelp" placeholder="Role Name">
                                <br>
                                @error('name')
                             ...
                             <div class="row">
                                    ...
                                    <div class="col-lg-9 mb-5">
                                        <div class="form-check">
                                            <!-- (all_permission) ei data shu ei input fild er jommo pathaichi -->
                                            <input type="checkbox"  class="form-check-input" id="allCheck"  
                                                {{ App\Models\User::roleHasPermissions($role,$all_permission)?'checked':'' }}
                                            >
                                ...
                                @foreach ($permissions_group as $group)
                                    @php 
                                        ...
                                        $j=1;
                                        $SingleGroupCheck=App\Models\User::roleHasPermissions($role,$permission)?'checked':'';
                                    @endphp
                                  
                                    <div class="row">
                                        <div class="col-lg-3 mb-2">
                                             <div class="form-check">
                                                <input type="checkbox" value="{{ $group->name}}"  class="form-check-input" id="{{ $i }}-management" onclick="checkPermissionByGroup('role-{{ $i }}-management-checkbox',this)" {{ App\Models\User::roleHasPermissions($role,$permission)?'checked':'' }}>
                                        ...
                                        <div class="col-lg-9 mb-5 role-{{ $i }}-management-checkbox">
                                            ...
                                            <!--  pakege er sathe thake-($role->hasPermissionTo) -->
                                            @foreach ($permission as $permissions)
                                                <div class="form-check">
                                                    <!-- ekhane ze (onclick) ta use kora hoyeche karon (zodu kuno checkbox uncheck thake thaole (all & group name) gula uncheck hoye zabe) -->
...
<script>
    ...
    // group name click check & UnCheck
